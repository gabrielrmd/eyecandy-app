...
INSERT INTO public.questionnaire_questions (section_id, question_number, question_text, question_type, required, help_text, placeholder) VALUES
((SELECT id FROM public.questionnaire_sections WHERE section_number = 1), 1, 'What is your company/brand name?', 'text', true, 'Enter your official business name', 'e.g., Acme Corp'),
...
INSERT INTO public.questionnaire_questions (section_id, question_number, question_text, question_type, required, help_text, placeholder) VALUES
((SELECT id FROM public.questionnaire_sections WHERE section_number = 2), 7, 'Who is your primary target audience/customer?', 'textarea', true, 'Describe your ideal customer in detail. Include demographics, psychographics, and pain points', 'e.g., Marketing managers at tech startups, aged 28-40, seeking to scale growth...'),
((SELECT id FROM public.questionnaire_sections WHERE section_number = 2), 8, 'What are the key pain points your target audience faces?', 'textarea', true, 'What problems do your customers struggle with?', 'e.g., Limited budget, too many tools to manage, difficulty measuring ROI...'),
...
((SELECT id FROM public.questionnaire_sections WHERE section_number = 2), 10, 'What is your customer acquisition cost (CAC)?', 'text', false, 'Approximate cost per customer or estimated range', 'e.g., €50-150 per customer'),
((SELECT id FROM public.questionnaire_sections WHERE section_number = 2), 11, 'What is your customer lifetime value (LTV)?', 'text', false, 'Approximate total value per customer over lifetime', 'e.g., €5000-15000'),
((SELECT id FROM public.questionnaire_sections WHERE section_number = 2), 12, 'What is your target market size/TAM?', 'textarea', false, 'Estimate your Total Addressable Market', 'e.g., €500M globally, €50M in Europe...');

-- Section 3: Competitive Landscape (Questions 13-18)
INSERT INTO public.questionnaire_questions (section_id, question_number, question_text, question_type, required, help_text, placeholder) VALUES
((SELECT id FROM public.questionnaire_sections WHERE section_number = 3), 13, 'Who are your top 3 competitors?', 'textarea', true, 'List and briefly describe your main competitors', 'e.g., Competitor A (focuses on enterprise, premium pricing), Competitor B...'),
((SELECT id FROM public.questionnaire_sections WHERE section_number = 3), 14, 'What are their main strengths?', 'textarea', true, 'What do competitors do well?', 'e.g., Strong brand recognition, excellent customer service, lower prices...'),
((SELECT id FROM public.questionnaire_sections WHERE section_number = 3), 15, 'What are their main weaknesses?', 'textarea', true, 'Where do competitors fall short?', 'e.g., Poor UX, expensive, lacks support, limited features...'),
((SELECT id FROM public.questionnaire_sections WHERE section_number = 3), 16, 'How do you differentiate from competitors?', 'textarea', true, 'What makes you unique and different?', 'e.g., Superior product quality, better customer support, lower cost...'),
...
((SELECT id FROM public.questionnaire_sections WHERE section_number = 3), 18, 'What competitive advantages are hardest to replicate?', 'textarea', false, 'Your moats or defensible assets', 'e.g., Proprietary technology, network effects, brand loyalty, patents...');

-- Section 4: Value Proposition (Questions 19-24)
INSERT INTO public.questionnaire_questions (section_id, question_number, question_text, question_type, required, help_text, placeholder) VALUES
((SELECT id FROM public.questionnaire_sections WHERE section_number = 4), 19, 'What is your core value proposition?', 'textarea', true, 'The main benefit/value you deliver. Keep it concise.', 'e.g., Save 10 hours/week on marketing tasks through AI automation'),
((SELECT id FROM public.questionnaire_sections WHERE section_number = 4), 20, 'What specific problems does your product/service solve?', 'textarea', true, 'List the key problems you address', 'e.g., Reduces marketing team time, improves campaign ROI, ensures brand consistency...'),
((SELECT id FROM public.questionnaire_sections WHERE section_number = 4), 21, 'What measurable results do customers achieve?', 'textarea', true, 'Quantifiable outcomes from using your solution', 'e.g., 40% increase in conversion rate, 25% cost reduction, 50% faster campaign launch...'),
((SELECT id FROM public.questionnaire_sections WHERE section_number = 4), 22, 'What is your key brand message/positioning?', 'textarea', true, 'How you want to be perceived in the market', 'e.g., "The intelligent alternative to traditional agencies"'),
...
((SELECT id FROM public.questionnaire_sections WHERE section_number = 4), 24, 'What are your top 3 key features or benefits?', 'textarea', true, 'Your most important differentiators', 'e.g., 1. AI-powered personalization, 2. One-click deployment, 3. Real-time analytics...');

-- Section 5: Marketing Goals & Metrics (Questions 25-30)
INSERT INTO public.questionnaire_questions (section_id, question_number, question_text, question_type, required, help_text, placeholder) VALUES
((SELECT id FROM public.questionnaire_sections WHERE section_number = 5), 25, 'What is your #1 marketing goal for the next 12 months?', 'textarea', true, 'Your primary objective', 'e.g., Increase brand awareness, grow customer base by 50%, launch new product...'),
((SELECT id FROM public.questionnaire_sections WHERE section_number = 5), 26, 'What is your revenue goal for next year?', 'text', false, 'Target annual revenue or growth percentage', 'e.g., €2M ARR or 100% growth'),
((SELECT id FROM public.questionnaire_sections WHERE section_number = 5), 27, 'What are your top 3 key performance indicators (KPIs)?', 'textarea', true, 'The metrics that matter most to your business', 'e.g., Customer acquisition cost, annual recurring revenue, customer lifetime value...'),
((SELECT id FROM public.questionnaire_sections WHERE section_number = 5), 28, 'What is your marketing budget for the next 12 months?', 'text', false, 'Annual marketing spend or percentage of revenue', 'e.g., €50K or 10% of revenue'),
((SELECT id FROM public.questionnaire_sections WHERE section_number = 5), 29, 'What marketing initiatives are currently underway?', 'textarea', false, 'Current campaigns, projects, or focuses', 'e.g., SEO optimization, LinkedIn lead generation, product launch campaign...'),
((SELECT id FROM public.questionnaire_sections WHERE section_number = 5), 30, 'What is your expected customer retention/churn rate?', 'text', false, 'Estimated percentage or timeframe', 'e.g., 90% annual retention or 3% monthly churn');

-- Section 6: Current Challenges & Gaps (Questions 31-36)
INSERT INTO public.questionnaire_questions (section_id, question_number, question_text, question_type, required, help_text, placeholder) VALUES
((SELECT id FROM public.questionnaire_sections WHERE section_number = 6), 31, 'What is your biggest marketing challenge right now?', 'textarea', true, 'Your most pressing pain point', 'e.g., Difficulty generating qualified leads, poor content performance, lack of resources...'),
...
((SELECT id FROM public.questionnaire_sections WHERE section_number = 6), 34, 'How satisfied are you with your current marketing performance?', 'scale', true, 'Rate your current results', 1, 10, 'Very Unsatisfied', 'Very Satisfied'),
((SELECT id FROM public.questionnaire_sections WHERE section_number = 6), 35, 'What has worked well in your marketing so far?', 'textarea', false, 'Your past successes and wins', 'e.g., Content marketing brought 40% of leads, LinkedIn outreach was effective...'),
((SELECT id FROM public.questionnaire_sections WHERE section_number = 6), 36, 'What has not worked or failed?', 'textarea', false, 'Past initiatives that underperformed', 'e.g., Paid Facebook ads were too expensive, email campaigns had low open rates...');

-- Section 7: Future Vision & Growth (Questions 37-39)
INSERT INTO public.questionnaire_questions (section_id, question_number, question_text, question_type, required, help_text, placeholder) VALUES
((SELECT id FROM public.questionnaire_sections WHERE section_number = 7), 37, 'Where do you see your business in 3 years?', 'textarea', true, 'Your vision and strategic direction', 'e.g., Market leader in our vertical with 50k customers, acquired by larger company...'),
((SELECT id FROM public.questionnaire_sections WHERE section_number = 7), 38, 'What is your biggest growth opportunity?', 'textarea', true, 'Your best avenue for rapid growth', 'e.g., Expanding to new markets, new product lines, strategic partnership...'),
((SELECT id FROM public.questionnaire_sections WHERE section_number = 7), 39, 'How can we best help you achieve your goals?', 'textarea', true, 'What support or expertise would be most valuable?', 'e.g., Help develop comprehensive marketing strategy, improve content, optimize sales funnel...');
